...
\subsubsection{Matrix of Dual Linear Map}

\begin{proposition}
    Suppose $V$ and $W$ are finite-dimensional and $T \in \LL(V,W)$. Then
    \[ \MM(T',(\psi_1,\ldots,\psi_m),(\varphi_1,\ldots,\varphi_n))=(\MM(T,(v_1,\ldots,v_n),(w_1,\ldots,w_n)))^t \]
    where $(\psi_1,\ldots,\psi_m)$ and $(\varphi_1,\ldots,\varphi_n)$ are the dual basis of $W'$ and $V'$ respectively.
\end{proposition}

\begin{proof}
Let $A = \mathcal{M}(T)$ and $C = \mathcal{M}(T')$. Suppose $1 \le j \le m$ and $1 \le k \le n$.
...
Thus $C = A^{\mathsf{t}}$. In other words, $\mathcal{M}(T') = \mathcal{M}(T)^{\mathsf{t}}$, as desired.
\end{proof}