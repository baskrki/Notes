...
\textit{Solution.} Using the fact, if $x_{n_k} \not \to x$ then there exists a $r>0$ such that $E \cap B(x,r)$ is finite.
Since no subsequence converges to $x_k$ for $k \in \NN$, we have $r>0$ s.t $E \cap B(x_k,r)$ is finite. 
Suppose
