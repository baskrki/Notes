...
\begin{align*}
    ...
    &\le \hspace{10mm} \vdots \\
    &\le |a_1-a_2| + \sum_{i=1}^{m-1} |a_i - a_{i+1}| \\
    &\le |a_1-a_2| + \sum_{i=1}^{m-1} \lambda^{i} |a_1-a_2| \\
    &\le |a_1-a_2| \left( \sum_{i=0}^{m-1} \lambda^i \right) \\
    &\le |a_1-a_2| \left(\frac{1-\lambda^{m}}{1-\lambda}\right) \le \left(\frac{1}{1-\lambda}\right)|a_1-a_2|
\end{align*}

For the second part, suppose $m > n$ then,
\begin{align*}
    |a_n-a_m| &\le |a_n-a_{m-1}|+|a_{m-1}-a_m| \\
    &\le \hspace{10mm} \vdots \\
    &\le \sum_{i=n}^{m-1} |a_i-a_{i+1}| \\
    &\le \sum_{i=n}^{m-1} \lambda^{i-1} |a_1-a_2| \\
    &\le |a_1-a_2| \cdot \left( \lambda^{n-1} \cdot \frac{1-\lambda^{m-n+2}}{1-\lambda} \right) \\
    &\le \frac{\lambda^{n-1}}{1-\lambda} |a_1-a_2| 
\end{align*}
So for any $\eps >0$ we can just choose a large $n$ such that $\dps \eps > \frac{\lambda^{n-1}}{1-\lambda} |a_1-a_2| $.